...
class CurrentStateWidget(QGroupBox):
    ...
    def __init__(self, ble_manager, parent=None):
        ...
        self.units = self.ble_manager.values_to_units_map
        self.labels = []
        ...
        for key in self.last_state.keys():
            label = QLabel(f"{key}: {self.last_state[key]} {self.units[key]}")
    ...
    def update(self, state_dict):
        ...
        for i, key in enumerate(self.last_state.keys()):
            if key == 'current':
                ...
                self.labels[i].setText(f"{key}: {avg_current:.2f} {self.units[key]}")
            elif key == 'voltage':
                self.battery_values.append(self.last_state[key])
                avg_voltage = sum(self.battery_values)/len(self.battery_values)
                self.labels[i].setText(f"{key}: {avg_voltage:.2f} {self.units[key]}")
            else:
                self.labels[i].setText(f"{key}: {self.last_state[key]} {self.units[key]}")
